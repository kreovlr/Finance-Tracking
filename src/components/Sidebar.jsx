...
export default function Sidebar() {
  ...
  const navigation = <><div className="mb-10 flex items-center justify-between"><h1 className="text-2xl font-bold text-purple-200">Finance Tracking</h1><button onClick={() => setOpen(false)} className="md:hidden"><X size={21} /></button></div><nav className="space-y-2">{links.map(({ to, label, icon: Icon }) => <Link key={to} to={to} onClick={() => setOpen(false)} className={`flex items-center gap-3 rounded-xl p-3 transition ${location.pathname === to ? "bg-purple-200/10 text-purple-200" : "text-slate-500 hover:bg-purple-200/10 hover:text-purple-200 dark:text-neutral-400"}`}><Icon size={20} />{label}</Link>)}</nav><button onClick={logout} className="mt-auto flex items-center gap-3 p-3 text-slate-500 hover:text-purple-300 dark:text-neutral-400"><LogOut size={20} />Sign out</button></>;
